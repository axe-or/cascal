...
fn interning_all_kinds_and_growth() {
    ...
    assert_eq!(int, type_intern(&mut arena, Type::Primitive(PrimitiveType::Int)));
    for ty in [Type::Pointer { inner: int }, Type::Slice { inner: int }, Type::Array { inner: int, size: 4 }, Type::Distinct { inner: int, name: "Index".to_owned() }] {
    ...
    for _ in 0..10000 { previous = type_intern(&mut arena, Type::Pointer { inner: previous }); }
    assert_eq!(type_arena_get(&arena, int), Some(&Type::Primitive(PrimitiveType::Int)));
...
fn collisions_are_resolved_by_equality() {
    ...
    let (a, b) = (0..300_000).find_map(|n| {
        let ty = Type::Distinct { inner: int, name: format!("name{n}") };
        let h = type_hash(&ty);
...
fn names_are_owned_and_lengths_matter() {
    ...
    let id = type_intern(&mut arena, Type::Distinct { inner: int, name: name.clone() });
    drop(name);
    assert_eq!(arena.types[id], Type::Distinct { inner: int, name: "Index".into() });
    let other = type_intern(&mut arena, Type::Distinct { inner: int, name: "Index2".into() });
    assert_ne!(id, other);
}
