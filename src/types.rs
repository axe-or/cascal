...
pub fn type_arena_get(arena: &TypeArena, id: TypeID) -> Option<&Type> {
    arena.types.get(id)
}
...
pub fn type_intern(arena: &mut TypeArena, ty: Type) -> TypeID {
    ...
    if let Some(id) = type_arena_find(arena, hash, &ty) {
        return id;
    }
    let inner = match &ty {
    ...
    if let Some(id) = inner {
        assert!(arena.types.get(id).is_some(), "invalid child type ID");
    }
