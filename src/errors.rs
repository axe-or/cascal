...
#[repr(u8)]
pub enum ErrorType {
    UnexpectedChar = 1, UnknownChar, InvalidBase, InvalidNumber,
    InvalidEscapeSequence, InvalidStringChar, UnclosedString, UnclosedComment,
    UnexpectedToken, MismatchedListCardinality,
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ErrorDetail { Character(char), TokenType(TokenType), Cardinality(usize) }
...
impl Error {
    pub fn new(kind: ErrorType, offset: usize) -> Self {
        Self { file: String::new(), offset, kind, expected: None, got: None }
    }
    pub fn character(kind: ErrorType, offset: usize, got: char) -> Self {
        Self { got: Some(ErrorDetail::Character(got)), ..Self::new(kind, offset) }
    }
}

impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}:{} error[E{:04}]: {}", self.file, self.offset, self.kind as u8, error_type_name(self.kind))
    }
}
