...
pub fn parser_make(source: &[u8]) -> Parser<'_> {
    Parser {
...
pub fn node_list_set_parent(ast: &mut AST, list: NodeList, parent: NodeID) {
    let mut current = list.first;
    while let Some(id) = current {
        ast.arena[id].parent = Some(parent);
        current = ast.arena[id].next;
    }
}
...
pub fn ast_make_node(ast: &mut AST, value: NodeValue) -> NodeID {
    ...
    let id = ast.arena.alloc(Node {
        parent: None,
        next: None,
        value,
    });
    for child in children.into_iter().flatten() {
        ast.arena[child].parent = Some(id);
    }
    for list in lists {
        node_list_set_parent(ast, list, id);
    }
    id
}
...
pub fn parse_type(parser: &mut Parser<'_>) -> ParserResult {
    ...
    let ty = match token.kind {
        ...
        SquareOpen => {
            if parser_take_if(parser, SquareClose)? {
                ParserType::Slice(parse_type(parser)?)
            } else {
...
pub fn parser_check_cardinality(
    parser: &Parser<'_>,
    left: NodeList,
    right: NodeList,
) -> Result<(), Error> {
...
pub fn parse_var_declaration(parser: &mut Parser<'_>) -> ParserResult {
    ...
    Ok(ast_make_node(
        &mut parser.ast,
        NodeValue::VarDefinition { idents, ty, values },
    ))
}
pub fn parse_field_list(parser: &mut Parser<'_>) -> Result<NodeList, Error> {
    ...
    loop {
        ...
        while let Some(id) = current {
            let NodeValue::Identifier(identifier) = &parser.ast.arena[id].value else {
                unreachable!()
            };
        ...
        if !parser_take_if(parser, TokenType::Comma)?
            || parser_peek(parser)?.kind == TokenType::ParenClose
        {
            break;
        }
    }
...
pub fn parse_proc_return_types(parser: &mut Parser<'_>) -> Result<NodeList, Error> {
    let mut list = NodeList::default();
    if !parser_take_if(parser, TokenType::Arrow)? {
        return Ok(list);
    }
    ...
    loop {
        ...
        if !grouped || !parser_take_if(parser, TokenType::Comma)? {
            break;
        }
    }
    if grouped {
...
pub fn parse_proc_definition(parser: &mut Parser<'_>) -> ParserResult {
    ...
    let args = if parser_peek(parser)?.kind == TokenType::ParenClose
        || parser_take_if(parser, TokenType::Comma)?
    {
        NodeList::default()
    } else {
...
pub fn parse_expression_or_assignment(parser: &mut Parser<'_>) -> ParserResult {
    ...
    if !parser_take_if(parser, TokenType::Assign)? {
        if left.first != left.last {
            return Err(parser_unexpected(parser_peek(parser)?, TokenType::Assign));
        }
    ...
    Ok(ast_make_node(
        &mut parser.ast,
        NodeValue::Assignment { left, right },
    ))
}
pub fn parse_return_statement(parser: &mut Parser<'_>) -> ParserResult {
    parser_expect(parser, TokenType::Return)?;
    let values = if parser_peek(parser)?.kind == TokenType::Semicolon {
        NodeList::default()
    } else {
        parse_expression_list(parser, TokenType::Semicolon)?
    };
...
pub fn parse_if_statement(parser: &mut Parser<'_>) -> ParserResult {
    ...
    let else_branch = if parser_take_if(parser, TokenType::Else)? {
        Some(if parser_peek(parser)?.kind == TokenType::If {
...
pub fn parse_statement(parser: &mut Parser<'_>) -> ParserResult {
    ...
    let node = match kind {
        ...
        Break | Continue => parse_branch_control(parser, kind),
        If => parse_if_statement(parser),
        While => parse_while_statement(parser),
        Proc => parse_proc_definition(parser),
...
pub fn node_format(writer: &mut impl Write, ast: &AST, id: NodeID) -> io::Result<()> {
    ...
    match &node.value {
        ...
        ParserType(ty) => match ty {
            ...
            crate::lang::ParserType::Slice(element) => {
                write!(writer, "[]")?;
                node_format(writer, ast, *element)
            }
            crate::lang::ParserType::Pointer(element) => {
                write!(writer, "^")?;
                node_format(writer, ast, *element)
        ...
        Unary { op, operand } => {
            ...
            node_format(writer, ast, *operand)?;
            write!(writer, ")")
        }
        Binary { op, left, right } => {
            write!(writer, "({} ", token_type_name(*op))?;
            node_format(writer, ast, *left)?;
            write!(writer, " ")?;
            node_format(writer, ast, *right)?;
            write!(writer, ")")
        }
        Index { object, idx } => {
            write!(writer, "([] ")?;
            node_format(writer, ast, *object)?;
            write!(writer, " ")?;
            node_format(writer, ast, *idx)?;
            write!(writer, ")")
        }
        Call { callable, args } => {
            write!(writer, "(call ")?;
            node_format(writer, ast, *callable)?;
            node_list_format(writer, ast, *args, true)?;
            write!(writer, ")")
        }
        Field { identifier, ty } => {
            write!(writer, "(field {identifier} ")?;
            node_format(writer, ast, *ty)?;
            write!(writer, ")")
        }
        VarDefinition { idents, ty, values } => {
            write!(writer, "(var (")?;
            node_list_format(writer, ast, *idents, false)?;
            write!(writer, ") ")?;
            node_format(writer, ast, *ty)?;
            write!(writer, " (")?;
            node_list_format(writer, ast, *values, false)?;
            write!(writer, "))")
        }
        Assignment { left, right } => {
            write!(writer, "(= (")?;
            node_list_format(writer, ast, *left, false)?;
            write!(writer, ") (")?;
            node_list_format(writer, ast, *right, false)?;
            write!(writer, "))")
        }
        Block { statements } => {
            write!(writer, "(block")?;
            node_list_format(writer, ast, *statements, true)?;
            write!(writer, ")")
        }
        Return { values } => {
            write!(writer, "(return")?;
            node_list_format(writer, ast, *values, true)?;
            write!(writer, ")")
        }
        ...
        If {
            ...
            write!(writer, "(if ")?;
            node_format(writer, ast, *condition)?;
            write!(writer, " ")?;
            node_format(writer, ast, *then_block)?;
        ...
        ProcDefinition {
            ...
            node_list_format(writer, ast, *args, false)?;
            write!(writer, ") (")?;
            node_list_format(writer, ast, *returns, false)?;
            write!(writer, ") ")?;
            node_format(writer, ast, *body)?;
            write!(writer, ")")
        }
